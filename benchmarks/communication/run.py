...
def tcp_ssh_into_host_run_server_and_client(hostip, username, port, file_size, threads, total_requests, password, log_file, output_file, isBW):
    ...
    command = ["bash", shell_script, username, hostip, str(port), str(file_size), str(threads), str(total_requests), file_path, output_file, password, isBW]
...
def run_benchmark(port, data_size, queue_depth, threads, test_rounds, host_ib_dev, dpu_ib_dev, host_username, host_ip, dpu_ip, password, output_folder, log_file, log_file_path, benchmark_item, metric_lst):
    ...
    metrics = metric_lst.split(',')

    if benchmark_item == "TCP":
        isBW = 1 if "bandwidth" in metrics else 0
        output_file = os.path.join(test_run_dir, f"latency_output.csv")
        
        tcp_ssh_into_host_run_server_and_client(host_ip, host_username, port, data_size, threads, test_rounds, password, log_file_path, temp_output_file, isBW)
    ...
    if benchmark_item == "RDMA":
        ...
        rdma_ssh_into_host_run_server_and_client(host_ip, host_username, port, data_size, threads, test_rounds, password, host_ib_dev, dpu_ib_dev, log_file_path, temp_output_file, "latency")
